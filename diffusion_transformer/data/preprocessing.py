...
from diffusion_transformer.data.data_loading_utils import load_match_events
from diffusion_transformer.data.event_autoencoder_model import EventAutoencoder


def load_and_embed_matches(args) -> tuple[dict[Any, Any], dict[Any, Any]]:
    """
    ...
    events_cache_path = Path(args.cache_dir) / "events_df.pkl"

    # Load all match events
    if events_cache_path and events_cache_path.exists() and not args.force_recompute:
        logger.info(f"Loading cached match events from {events_cache_path}")
        events_df = pd.read_pickle(events_cache_path)
    else:
        logger.info(f"Loading match events from {args.csv_dir}")
        events_df = load_match_events(csv_root_dir=args.csv_dir)
    ...
    if args.task:
        logger.info(f"Using task-specific caching for task: '{args.task}'")

    mask_list = None
    if args.task in ["shot_only"]:
    ...
    model.load_state_dict(torch.load(args.autoencoder_path, map_location=device, weights_only=False))
    model = model.to(device)
    model.eval()

    # Process each match
    for match_id in tqdm(match_ids, desc="Processing matches", disable=not args.verbose):
        ...
        if args.cache_dir:
            ...
            embeddings_cache_path = Path(args.cache_dir) / f"{match_id}_embeddings{task_suffix}.npy"
            events_cache_path = Path(args.cache_dir) / f"{match_id}_events.npy"  # Events are always the same

        # Check if embeddings are cached (task-specific caching ensures consistency)
        use_cached_embeddings = (embeddings_cache_path and embeddings_cache_path.exists() and not args.force_recompute)
        use_cached_events = (events_cache_path and events_cache_path.exists() and not args.force_recompute)
        ...
        if use_cached_embeddings and use_cached_events:
            ...
            cache_info = f"task='{args.task}'" if args.task else "default"
            ...
            if mask_list is not None:
                ...
                for mask_idx in mask_list:
                    if mask_idx < match_data_for_embedding.shape[1]:
                        ...
                        logger.warning(
                            f"Mask index {mask_idx} is out of bounds for {match_data_for_embedding.shape[1]} columns")
            ...
            match_embeddings = []
            with torch.no_grad():
                for i in range(0, len(match_tensor), args.batch_size):
                    batch = match_tensor[i:i + args.batch_size].to(device)
...
def get_class_weights(dataset_or_loader: torch.utils.data.Dataset | torch.utils.data.DataLoader) -> torch.Tensor:
    """
